...
class GameModelView: ObservableObject {
    ...
    /// Nested struct for camera-specific state and functions.
    class CameraState {
        // MARK: - Camera Control Properties

        var cameraAngle: Angle = .zero
        var targetCameraAngle: Angle = .zero
        var cameraDistance: Float = 7.0
        var targetCameraDistance: Float = 7.0

        // MARK: - Pitch Control

        var cameraPitch: Angle = .degrees(0)
        var targetCameraPitch: Angle = .degrees(0)
        ...
        // MARK: - RealityKit Entity References
        ...
        func addCamera(to content: RealityViewCameraContent, relativeTo tracked: Entity) {
            ...
            // Attach the CameraRotationComponent so that the CameraRotationSystem can update it.
        ...
        func startSmoothCameraAnimation() {
            // Smoothing/damping logic to update cameraAngle, cameraPitch, and cameraDistance toward their targets.
