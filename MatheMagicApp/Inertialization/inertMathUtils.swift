...
/* G3: version
 ...
 func clamp<T: Comparable>(_ value: T, _ minVal: T, _ maxVal: T) -> T {
     Swift.max(minVal, Swift.min(value, maxVal))
 }

 /// Computes polynomial coefficients for fifth-order interpolation
 /// - Parameters:
 ///   - x0: Initial offset (position or angle)
 ///   - v0: Initial velocity
 ///   - tf: Desired blend time
 ///   - preventOvershoot: Toggle for overshoot prevention (preserved from C3)
 /// - Returns: Tuple of coefficients (A, B, C, a0) and effective blend time
 func computePolynomialCoefficients(x0: Float, v0: Float, tf: Float, preventOvershoot: Bool = true) -> (A: Float, B: Float, C: Float, a0: Float, effectiveTf: Float) {
     // Cache check moved to Caches.swift for centralization
     let key = PolynomialCacheKey(x0: x0, v0: v0, tf: tf, preventOvershoot: preventOvershoot)
     if let cached = PolynomialCache.shared.get(key) {
         return cached
     }

     // Early exit for negligible offset
     guard abs(x0) >= Epsilon.position else {
         return (0, 0, 0, 0, 0)
     }

     var adjustedTf = tf
     // Overshoot prevention from C3, only applied when configured and necessary
     if preventOvershoot && v0 * x0 < 0 {
         let tfCandidate = abs(5 * x0 / v0)
         if tfCandidate > 0 {
             adjustedTf = min(tf, tfCandidate)
         }
     }

     // Early exit for tiny blend times
     guard adjustedTf >= Epsilon.blendTime else {
         return (0, 0, 0, 0, 0)
     }

     // Precompute powers for efficiency (inspired by O3)
     let tf2 = adjustedTf * adjustedTf
     let tf3 = tf2 * adjustedTf
     let tf4 = tf3 * adjustedTf
     let tf5 = tf4 * adjustedTf

     // Compute coefficients (aligned with O3’s order, preserving C3’s logic)
     let a0 = (-8 * v0 * adjustedTf - 20 * x0) / tf2
     let A = -(a0 * tf2 + 6 * v0 * adjustedTf + 12 * x0) / (2 * tf5)
     let B = (3 * a0 * tf2 + 16 * v0 * adjustedTf + 30 * x0) / (2 * tf4)
     let C = -(3 * a0 * tf2 + 12 * v0 * adjustedTf + 20 * x0) / (2 * tf3)

     let result = (A, B, C, a0, adjustedTf)
     PolynomialCache.shared.set(key: key, value: result)
     return result
 }

 /// Evaluates a fifth-order polynomial at time t
 @inline(__always)
 func evaluatePolynomial(t: Float, maxT: Float, A: Float, B: Float, C: Float, a0: Float, v0: Float, x0: Float) -> Float {
     let clampedT = min(t, maxT)
     let t2 = clampedT * clampedT
     let t3 = t2 * clampedT
     let t4 = t3 * clampedT
